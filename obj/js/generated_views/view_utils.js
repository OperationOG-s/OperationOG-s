        ...
        if (this.state.kind == "menu") {
            return React.createElement("div", { className: "topbar__action-wrapper" },
                current_logged_in_entity ?
                    React.createElement(LanguageSelector, { current_User: this.props.current_User, current_Admin: this.props.current_Admin, set_User: u => this.props.set_User(u), set_Admin: u => this.props.set_Admin(u) })
                    :
                        null,
            ...
            return React.createElement("div", { className: "authentication-menu" },
                ...
                React.createElement("div", { className: "overlay", onClick: e => e.target == e.currentTarget && this.setState({ kind: "menu" }) },
                    React.createElement("div", { className: "authentication-menu__popup" },
                        ...
                        React.createElement("a", { className: "button-login", onClick: () => {
                                this.setState(Object.assign({}, this.state, { kind: "logging-in", action: (username, password) => Api.login_Admin(username, username, password).then(e => this.setState({ kind: "menu" }, () => this.props.set_Admin(e))).catch(error("Login error. Please check your username or email.")), reset_action: (username) => Api.reset_Admin_password(username, username).then(() => location.reload() || this.setState(Object.assign({}, this.state, { kind: "menu" }))).catch(error("Error: please check your username or email.")) }));
                            } }, i18next.t('Login as Admin')),
                        React.createElement("a", { className: "button-signup", onClick: () => {
                                this.setState(Object.assign({}, this.state, { kind: "registering", action: (username, email, email_confirmation) => email != email_confirmation ?
                                        error("Email and email confirmation do not match.")()
                                        :
                                            Api.register_User(username, email, email_confirmation).then(e => this.setState({ kind: "menu" }, () => this.props.set_User(e))).catch(error("Server error.")) }));
                            } }, i18next.t('Register as User')))));
        }
                ...
                React.createElement("div", { className: "overlay", onClick: e => e.target == e.currentTarget && this.setState({ kind: "menu" }) },
                    React.createElement("div", { className: "authentication-menu__popup" },
                                ...
                                let action = (password, new_password, new_password_confirmation) => new_password_confirmation != new_password ?
                                    ...
                                    :
                                        this.props.current_User ? Api.change_User_password(this.props.current_User.Username, this.props.current_User.Email, password, new_password, new_password_confirmation).then(() => this.setState({ kind: "menu" })).catch(error("Server error.")) :
                                            this.props.current_Admin ? Api.change_Admin_password(this.props.current_Admin.Username, this.props.current_Admin.Email, password, new_password, new_password_confirmation).then(() => this.setState({ kind: "menu" })).catch(error("Server error.")) :
                                                null;
                        ...
                        React.createElement("a", { onClick: () => this.props.current_User ? Api.logout_User().then(() => this.setState({ kind: "menu" }, () => this.props.set_User(null))) :
                                this.props.current_Admin ? Api.logout_Admin().then(() => this.setState({ kind: "menu" }, () => this.props.set_Admin(null))) :
        ...
        return this.state.element ?
            React.createElement("div", { className: "scene" },
                ...
                this.state.current_renderer({
                    current_User: this.props.current_User, current_Admin: this.props.current_Admin,
...
class PageManager extends React.Component {
    constructor(props, context) {
        ...
        this.state = {
            ...
            current_User: props.current_User, current_Admin: props.current_Admin, logic_frame: 0
        };
    }
    componentWillMount() {
        ...
        this.keep_alive_thread = setInterval(() => {
            ...
            else if (this.props.current_Admin)
                KeepAliveApi.ping_as_Admin().then(() => this.state.connection != "connected" && this.setState(Object.assign({}, this.state, { connection: "connected" }))).catch(() => {
                    if (this.state.connection == "maybe-disconnected1")
                        this.setState(Object.assign({}, this.state, { connection: "maybe-disconnected2" }));
                    else if (this.state.connection == "maybe-disconnected2")
                        this.setState(Object.assign({}, this.state, { connection: "disconnected" }));
    ...
    render() {
        let authentication_menu = () => React.createElement(AuthenticationMenu, { current_User: this.state.current_User, current_Admin: this.state.current_Admin, set_User: (new_User) => this.setState(Object.assign({}, this.state, { current_User: new_User }), () => location.reload()), set_Admin: (new_Admin) => this.setState(Object.assign({}, this.state, { current_Admin: new_Admin }), () => location.reload()) });
        let breadcrumbs = () => React.createElement("div", { className: "breadcrumbs" }, this.state.pages.count() == 1 ?
            ...
            :
                this.state.pages.map((p, i) => React.createElement("a", { key: `${i}`, className: "breadcrumbs__item", style: Object.assign({}, (i == this.state.pages.count() - 1 ?
                    ...
                    } }, p.render(true)(false)(this.state.pages.count(), this.state.logic_frame, (c) => c && c())(this.state.current_User, this.state.current_Admin)(p.shown_relation, np => { })(authentication_menu, breadcrumbs)(Immutable.Stack())((np, c) => { }, (np, c) => { }, c => { }))).reverse());
...
function render_page_manager(target_element_id, initial_page, current_User, current_Admin) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        i18next.init({
            ...
            ReactDOM.render(React.createElement(PageManager, { initial_page: initial_page, current_User: current_User, current_Admin: current_Admin }), document.getElementById(target_element_id));
        });
    });
}
