...
export let any_of = (predicates:Array<((current_User:Models.User,
  current_Admin:Models.Admin) => boolean)>) =>
  (current_User:Models.User,
  current_Admin:Models.Admin) =>
  predicates.map(p => p(current_User, current_Admin)).some(p => p)
      ...
      return <div className="topbar__action-wrapper">
        {
          current_logged_in_entity ?
            <LanguageSelector current_User={this.props.current_User} current_Admin={this.props.current_Admin}
                              set_User={u => this.props.set_User(u)} set_Admin={u => this.props.set_Admin(u)} />
          :
            null
        }
      ...
      return <div className="authentication-menu">
        ...
        <a className="button-login"
                      onClick={() => {
                        this.setState({...this.state, kind:"logging-in",
                          action:(username, password) =>
                            Api.login_Admin(username, username, password).then(e =>
                              this.setState({kind:"menu"}, () =>
                                this.props.set_Admin(e))
                            ).catch(error("Login error. Please check your username or email.")),
                          reset_action:(username:string) =>
                            Api.reset_Admin_password(username, username).then(() => location.reload() || this.setState({...this.state, kind:"menu"})).catch(error("Error: please check your username or email."))
                        })
                      }
                      }>
            ...
            <a className="button-signup"
                      onClick={() => {
                        this.setState({...this.state, kind:"registering",
                          action:(username, email, email_confirmation) =>
                            email != email_confirmation ?
                              error("Email and email confirmation do not match.")()
                            :
                            Api.register_User(username, email, email_confirmation).then(e =>
                              this.setState({kind:"menu"}, () =>
                                this.props.set_User(e)
                              )
              ...
              <a
                      onClick={() => {
                        let action = (password:string, new_password:string, new_password_confirmation:string) =>
                                      ...
                                      this.props.current_Admin ? Api.change_Admin_password(this.props.current_Admin.Username, this.props.current_Admin.Email, password, new_password, new_password_confirmation).then(() =>
                                        this.setState({kind:"menu"})).catch(error("Server error.")) : 
                                      null
              ...
              <a
                    onClick={() =>
                      this.props.current_User ? Api.logout_User().then(() =>
                        ...
                        this.props.current_Admin ? Api.logout_Admin().then(() =>
                          this.setState({kind:"menu"}, () =>
...
type SceneProps<T> = { initial_renderer : Renderer<T>, get_element:Promise<Api.ItemWithEditable<T>>, save_element:(new_element:T)=>Promise<void>, can_edit:boolean,
                       ...
                       current_User:Models.User, current_Admin:Models.Admin, logic_frame:number, force_reload:(callback?:()=>void) => void }
...
export function scene_to_page<T>(can_edit:(current_User:Models.User, current_Admin:Models.Admin) => boolean, renderer:Renderer<T>,
  ...
  return {
    render: (is_breadcrumb:boolean) => (is_animating:boolean) => (pages_count:number, logic_frame:number, force_reload:(callback?:()=>void) => void) => (current_User:Models.User, current_Admin:Models.Admin) =>
        (shown_relation:string, set_shown_relation:(new_shown_relation:string, callback?:()=>void)=>void) =>
        (authentication_menu:() => JSX.Element, breadcrumbs:() => JSX.Element) =>
        (nested_entity_names:Immutable.Stack<string>) =>
...
export type PageRenderer = (is_breadcrumb:boolean) => (is_animating:boolean) => (pages_count:number, logic_frame:number, force_reload:(callback?:()=>void) => void) => (current_User:Models.User, current_Admin:Models.Admin) =>
  (shown_relation:string, set_shown_relation:(new_shown_relation:string, callback?:()=>void)=>void) =>
  (authentication_menu:() => JSX.Element, breadcrumbs:() => JSX.Element) =>
  (nested_entity_names:Immutable.Stack<string>) =>
...
export type PageManagerProps = { initial_page: Page, current_User:Models.User,
  current_Admin:Models.Admin }
export type PageManagerState = { connection:"connected"|"maybe-disconnected1"|"maybe-disconnected2"|"disconnected", pages: Immutable.Stack<Page & {shown_relation:string}>, current_User:Models.User,
  current_Admin:Models.Admin, logic_frame:number }
export class PageManager extends React.Component<PageManagerProps, PageManagerState> {
  ...
  render() {
    let authentication_menu = () => <AuthenticationMenu
      current_User={this.state.current_User} current_Admin={this.state.current_Admin}
      ...
      set_Admin={(new_Admin:Models.Admin) =>
        this.setState({...this.state, current_Admin:new_Admin}, () => location.reload())} />
    let breadcrumbs = () => <div className="breadcrumbs">
        {
          ...
          this.state.pages.map((p, i) => <a
              ...
              {p.render(true)(false)(this.state.pages.count(), this.state.logic_frame, (c) => c && c())
                       (this.state.current_User, this.state.current_Admin)
                       (p.shown_relation, np => {})
...
export async function render_page_manager(target_element_id:string, initial_page:Page, current_User:Models.User, current_Admin:Models.Admin) {
  ...
  i18next.init({
    ...
    ReactDOM.render(
      <PageManager initial_page={initial_page} current_User={current_User} current_Admin={current_Admin} />,
